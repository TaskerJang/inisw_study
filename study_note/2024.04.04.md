## 1. sklearn datasets => make, fetch, load => 3가지의 데이터 종류 존재

## 2. skipna 옵션에 따라서 nan이 하나라도 있으면 합계가 계산 안될 수 있다.

## 3. a, b 테스트

## 4. fillna => 옵션듣 존재 => method : backfill, padfill => 뒤에 것을 덮어쓰는지 앞에 것을 덮어쓰는지에 따라 다르다.

## 5. 원하는 데이터를 필터링을 한 뒤 채울지 안채울지를 결정해야 한다.

## 6. count => missing data가 아닌 것의 갯수를 구하는것.

## 7. missing data => masking 된 데이터

## 8. map으로는 nan 데이터 처리 불가

## 9. fit_transform => skilearn에서 제공하는 imputer 방식

## 10. 인코딩

## 11. 라벨 인코딩

## 12. inverse_transform => skilearn -> 값을 통해 인덱스를 추출
...
## 17. train, test => 홀드 아웃 => 검증 필요

```python

NN => Matrix 곱
minimize_w(eight) Loss => 1. SE => -(Y-Yhat) = Yhat-Y
2. BCE/NLL => P
3. Softmax => P, P, ...
import numpy as np
...
plt.plot(loss)
[<matplotlib.lines.Line2D at 0x12fbad510>]

logistic(X@W+B)[:,:1] > .5, (X@W+B)[:,1:]
(array([[False],
        [ True],
        [ True],
        [ True]]),
...
2.780733130929609
plt.plot(loss)
[<matplotlib.lines.Line2D at 0x1731bd450>]
